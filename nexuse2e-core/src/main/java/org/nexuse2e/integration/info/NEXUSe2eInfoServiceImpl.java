...
public class NEXUSe2eInfoServiceImpl implements NEXUSe2EInfo {
    ...
    private static Messages convert(List<MessagePojo> msgs, boolean includePayload,
                                    boolean includeOptionalAttributes) throws DatatypeConfigurationException {
        ...
        for (MessagePojo mp : msgs) {
            ...
            ParticipantPojo localPartner =
                    Engine.getInstance().getCurrentConfiguration().getParticipantFromChoreographyByPartner(mp.getConversation().getChoreography(), mp.getConversation().getPartner());
    ...
    private static List<Conversation> convert(List<ConversationPojo> cps) throws DatatypeConfigurationException,
            NexusException {
        ...
        for (ConversationPojo cp : cps) {
            ...
            Messages messages = convert(Engine.getInstance().getTransactionService().getMessagesFromConversation(cp),
                    false, false);
            c.setMessages(messages);
            c.setModifiedDate(date(cp.getModifiedDate()));
            ParticipantPojo localPartner =
                    Engine.getInstance().getCurrentConfiguration().getParticipantFromChoreographyByPartner(cp.getChoreography(), cp.getPartner());
            if (localPartner != null && localPartner.getLocalPartner() != null && localPartner.getLocalPartner().getPartnerId() != null) {
    ...
    public List<LogMessage> getLogMessages(int pastMinutes, Integer maximumMessages) {
        ...
        List<LogPojo> l = null;
        try {
            l = service.getLogEntriesForReport(null, null, cal.getTime(), new Date(), (maximumMessages == null ?
                    Integer.MAX_VALUE : maximumMessages), 0, LogDAO.SORT_CREATED, true);
        } catch (NexusException nex) {
            nex.printStackTrace();
        }
        ...
        try {
            if (l != null) {
                for (LogPojo logPojo : l) {
                    ...
                    try {
                        m.setLogLevel(LogLevel.fromValue(StandardLevel.getStandardLevel(logPojo.getSeverity()).toString()));
                    } catch (IllegalArgumentException ignored) {
    ...
    public List<String> getActions(String choreography) {
        ...
        try {
            ChoreographyPojo cp =
                    Engine.getInstance().getCurrentConfiguration().getChoreographyByChoreographyId(choreography);
            if (cp != null) {
    ...
    public Partners getPartners(Object getPartners) {
        ...
        try {
            ...
            List<PartnerPojo> partners =
                    Engine.getInstance().getCurrentConfiguration().getPartners(org.nexuse2e.configuration.Constants.PARTNER_TYPE_PARTNER, comp);
    ...
    public Partners getLocalPartners(Object getPartners) {
        ...
        try {
            Comparator<PartnerPojo> comp = new GenericComparator<PartnerPojo>("name", true);
            List<PartnerPojo> partners =
                    Engine.getInstance().getCurrentConfiguration().getPartners(org.nexuse2e.configuration.Constants.PARTNER_TYPE_LOCAL, comp);
            List<Partner> result = p.getPartner();
            if (partners != null) {
                for (PartnerPojo pp : partners) {
                    Partner partner = new Partner();
    ...
    public String sendNewStringMessage(String choreographyId, String businessPartnerId, String actionId,
        ...
        try {
            return new NEXUSe2eInterfaceImpl().sendNewStringMessage(choreographyId, businessPartnerId, actionId, null
                    , payload);
        } catch (NexusException e) {
            e.printStackTrace();
        }
    ...
    /* (non-Javadoc)
     * @see org.nexuse2e.integration.info.wsdl.NEXUSe2EInfo#getConversationCount(org.nexuse2e.integration.info.wsdl
     * .ConversationFilter)
     */
    public int getConversationCount(ConversationFilter conversationFilter) {
        ...
        try {
            ...
            count = Engine.getInstance().getTransactionService().getConversationsCount(cfp.status,
                    cfp.nxChoreographyId, cfp.nxPartnerId, cfp.conversationId, cfp.afterDate, cfp.beforeDate, 0,
                    cfp.asc);
        } catch (NexusException nex) {
            nex.printStackTrace();
        }
    ...
    /* (non-Javadoc)
     * @see org.nexuse2e.integration.info.wsdl.NEXUSe2EInfo#getConversations(org.nexuse2e.integration.info.wsdl
     * .ConversationFilter)
     */
    public List<Conversation> getConversations(ConversationFilter conversationFilter) {
        try {
            ConversationFilterParams cfp = new ConversationFilterParams(conversationFilter);
            List<ConversationPojo> cp =
                    Engine.getInstance().getTransactionService().getConversationsForReport(cfp.status,
                            cfp.nxChoreographyId, cfp.nxPartnerId, cfp.conversationId, cfp.afterDate, cfp.beforeDate,
                            cfp.itemsPerPage, cfp.page, TransactionDAO.SORT_CREATED, cfp.asc);
            return convert(cp);
        } catch (NexusException nex) {
            nex.printStackTrace();
    ...
    /* (non-Javadoc)
     * @see org.nexuse2e.integration.info.wsdl.NEXUSe2EInfo#getMessageCount(org.nexuse2e.integration.info.wsdl
     * .MessageFilter)
     */
    public int getMessageCount(MessageFilter messageFilter) {
        ...
        try {
            ...
            count = Engine.getInstance().getTransactionService().getMessagesCount(cfp.status, null,
                    cfp.nxChoreographyId, cfp.nxPartnerId, cfp.conversationId, cfp.messageId, cfp.afterDate,
                    cfp.beforeDate);
        } catch (NexusException nex) {
            nex.printStackTrace();
        }
    ...
    public Messages getMessages(MessageFilter messageFilter) {
        try {
            MessageFilterParams cfp = new MessageFilterParams(messageFilter);
            List<MessagePojo> mp = Engine.getInstance().getTransactionService().getMessagesForReport(cfp.status,
                    cfp.nxChoreographyId, cfp.nxPartnerId, cfp.conversationId, cfp.messageId, null, cfp.afterDate,
                    cfp.beforeDate, cfp.itemsPerPage, cfp.page, TransactionDAO.SORT_CREATED, cfp.asc);
    ...
    public GetConversationLogMessageCountResponse getConversationLogMessageCount(GetConversationLogMessageCount getConversationLogMessageCount) {
        ...
        try {
            resp.setCount(Engine.getInstance().getTransactionService().getLogEntriesForReportCount(null,
                    getConversationLogMessageCount.getConversationId(), getConversationLogMessageCount.getMessageId()));
        } catch (NexusException e) {
            e.printStackTrace();
        }
        return resp;
    ...
    public LogMessages getConversationLogMessages(GetConversationLogMessages getConversationLogMessages) {
        ...
        try {
            ...
            List<LogPojo> entries = Engine.getInstance().getTransactionService().getLogEntriesForReport(null,
                    getConversationLogMessages.getConversationId(), getConversationLogMessages.getMessageId(),
                    itemsPerPage, page, false);
            lm = new LogMessages();
    ...
    private static class MessageFilterParams {
        ...
        public MessageFilterParams(MessageFilter messageFilter) throws NexusException {
            ...
            if (messageFilter.getChoreographyId() != null && messageFilter.getChoreographyId().getValue() != null) {
                ChoreographyPojo cp =
                        Engine.getInstance().getCurrentConfiguration().getChoreographyByChoreographyId(messageFilter.getChoreographyId().getValue());
                if (cp != null) {
            ...
            if (messageFilter.getBusinessPartnerId() != null && messageFilter.getBusinessPartnerId().getValue() != null) {
                PartnerPojo pp =
                        Engine.getInstance().getCurrentConfiguration().getPartnerByPartnerId(messageFilter.getBusinessPartnerId().getValue());
                if (pp != null) {
    ...
    private static class ConversationFilterParams {
        ...
        public ConversationFilterParams(ConversationFilter conversationFilter) throws NexusException {
            ...
            if (conversationFilter.getChoreographyId() != null && conversationFilter.getChoreographyId().getValue() != null) {
                ChoreographyPojo cp =
                        Engine.getInstance().getCurrentConfiguration().getChoreographyByChoreographyId(conversationFilter.getChoreographyId().getValue());
                if (cp != null) {
                    nxChoreographyId = cp.getNxChoreographyId();
                } else {
                    nxChoreographyId = Integer.MAX_VALUE; // assume this one doesn't exist
            ...
            if (conversationFilter.getBusinessPartnerId() != null && conversationFilter.getBusinessPartnerId().getValue() != null) {
                PartnerPojo pp =
                        Engine.getInstance().getCurrentConfiguration().getPartnerByPartnerId(conversationFilter.getBusinessPartnerId().getValue());
                if (pp != null) {
                    nxPartnerId = pp.getNxPartnerId();
                } else {
                    nxPartnerId = Integer.MAX_VALUE; // assume this one doesn't exist
