...
public abstract class ChoreographyValidator {
    ...
    private ChoreographyPojo getMatchingChoreography(List<ChoreographyPojo> choreographies, MessagePojo messagePojo) throws NexusException {
        ...
        while (i.hasNext()) {
            ...
            if (messagePojo.getConversation().getChoreography().getName() == null || messagePojo.getConversation().getChoreography().getName().equals("")) {
                LOG.error("invalid message, no choreographyId found in message header (messageId:" + messagePojo.getMessageId() + ")");
    ...
    public ChoreographyPojo validateChoreography(MessageContext messageContext) throws NexusException {
        ...
        if (messagePojo.getConversation() != null) {
            if (LOG.isTraceEnabled()) {
                ...
                LOG.trace(new LogMessage("Choreography:" + messagePojo.getConversation().getChoreography(),
                        messagePojo));
            }
        ...
        if (choreography == null) {
            throw new NexusException("No matching choreography found for choreographyId: " + messagePojo.getConversation().getChoreography().getName());
        }
    ...
    private PartnerPojo getMatchingPartner(List<PartnerPojo> partners, MessagePojo messagePojo) throws NexusException {
        ...
        while (i.hasNext()) {
            ...
            if (messagePojo.getConversation().getPartner().getPartnerId() == null || messagePojo.getConversation().getPartner().getPartnerId().equals("")) {
                LOG.error("invalid message, no partnerId found in message header (messageId:" + messagePojo.getMessageId() + ")");
    ...
    public ParticipantPojo validateParticipant(MessageContext messagePipelineParameter) throws NexusException {
        ...
        while (i.hasNext()) {
            ...
            if (participant.getPartner().getPartnerId().equals(messagePojo.getConversation().getPartner().getPartnerId())) {
                messagePipelineParameter.setParticipant(participant);
                return participant;
            }
        }

        throw new NexusException("No matching participant found for partner: " + messagePojo.getConversation().getPartner().getPartnerId());
    } // validateParticipant
