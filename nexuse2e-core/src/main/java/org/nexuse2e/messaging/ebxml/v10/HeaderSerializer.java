...
public class HeaderSerializer extends AbstractPipelet {
    ...
    //        System.setProperty( "javax.xml.soap.SOAPFactory", "com.sun.xml.messaging.saaj.soap.ver1_1
    //        .SOAPFactory1_1Impl" );
    ...
    public MessageContext processMessage(MessageContext messageContext) throws NexusException {
        ...
        try {
            ...
            if ((messagePojo.getType() == Constants.INT_MESSAGE_TYPE_ACK) && (messagePojo.getHeaderData() != null) && (messagePojo.getHeaderData().length != 0)) {
                ...
                // BODY ATTRS ----------------------------------------------------------
                // soapBody.addAttribute( soapFactory.createName( "xsi:schemaLocation" ), "http://www.ebxml.org/namespaces/messageHeader
                // http://www.ebxml.org/namespaces/messageHeader" );
                ...
                // CPA & ConversationId ------------------------------------------------
                createSOAPElement(soapFactory, msgHeader, "CPAId",
                ...
                if (ack || error) {
                    ...
                    createSOAPElement(soapFactory, msgDataEl, "RefToMessageId",
                ...
                if (ack) { // ack
                    ...
                    if (messagePojo.getParticipant().getConnection().isReliable()) {
                        SOAPElement soapElement = msgHeader.addChildElement("QualityOfServiceInfo",
                    ...
                    SOAPElement traceHeader = traceHeaderList.addChildElement("TraceHeader",
                            Constants.EBXML_NAMESPACE_PREFIX, Constants.EBXML_NAMESPACE);
                    SOAPElement sender = createPartyElement(soapFactory, "Sender", from, fromIdType, null);
                    Set<ConnectionPojo> localConnections =
                            messagePojo.getParticipant().getLocalPartner().getConnections();
                    ...
                    if (!localConnections.isEmpty()) {
                        ...
                        for (ConnectionPojo connectionPojo : localConnections) {
                            if (connectionPojo.getName().equalsIgnoreCase("ebxml10") || connectionPojo.getName().equalsIgnoreCase("ebms10") || connectionPojo.getUri().contains("ebxml10")) {
                    ...
                    createSOAPElement(soapFactory, receiver, "Location",
                            messagePojo.getParticipant().getConnection().getUri());
                    ...
                    for (MessagePayloadPojo bodyPart : messagePojo.getMessagePayloads()) {
                        ...
                        createManifestReference(soapFactory, soapManifest, bodyPart.getContentId(),
    ...
    private void createManifestReference(SOAPFactory soapFactory, SOAPElement parent, String href, String id,
        ...
        //soapEl.addAttribute( soapFactory.createName( "xmlns:" + Constants.EBXML_NAMESPACE_PREFIX ), Constants
        // .EBXML_NAMESPACE );
        ...
        if (type != null && type.length() != 0) {
            soapEl.addAttribute(soapFactory.createName("xlink:type"), "simple"); // hard coded according to spec, p.
            // 35, 8.11.3
        }
    }
     ...
     *
     */
    private void createSOAPElement(SOAPFactory soapFactory, SOAPElement parent, String childName, String childText) throws SOAPException {
    ...
    private SOAPElement createPartyElement(SOAPFactory soapFactory, String id, String value, String type,
        ...
        if (type != null && !type.equals("")) {
            ...
            // partyId.addAttribute( soapFactory.createName( "type", Constants.EBXML_NAMESPACE_PREFIX, Constants
            // .EBXML_NAMESPACE ), type );
            party = value;
