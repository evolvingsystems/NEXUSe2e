...
public class StatusUpdateSerializer implements Manageable {
    ...
    private static Logger LOG = LogManager.getLogger(StatusUpdateSerializer.class);
    ...
    public MessageContext processMessage(MessageContext messageContext) throws NexusException {
        ...
        LOG.debug(new LogMessage("StatusUpdateSerializer.processMessage - " + choreographyId,
        ...
        if (status == BeanStatus.STARTED) {
            ...
            LOG.error(new LogMessage("Received message for StatusUpdateSerializer (" + choreographyId + ") which " +
                    "hasn't been properly started!", messageContext.getMessagePojo()));
        }
    ...
    public boolean validate() {
        ...
        if ((backendInboundDispatcher != null) && (stateMachineExecutor != null) && (queue != null) && (statusUpdateQueueListener != null)) {
            return true;
        }
        ...
        public void run() {
            ...
            while (!stopRequested) {
                try {
                    ...
                    LOG.debug(new LogMessage("###############  Status Update #################",
                            messageContext.getMessagePojo()));
                    ...
                    if (statusUpdatePipelines != null) {
                        ...
                        ActionSpecificKey actionSpecificKey = new ActionSpecificKey((action == null ? null :
                                action.getName()), (choreography == null ? null : choreography.getName()));
                        BackendPipeline statusUpdatePipeline = statusUpdatePipelines.get(actionSpecificKey);
                        if (statusUpdatePipeline != null) {
                            LOG.debug(new LogMessage("Found pipeline: " + statusUpdatePipeline + " - " + actionSpecificKey, messageContext.getMessagePojo()));
                            ...
                            try {
                                ...
                                LOG.error(new LogMessage("Error cloning original MessagePojo!",
                                        messageContext.getMessagePojo()));
                            }
                            ...
                            LOG.debug(new LogMessage("No status pipeline found for message: " + messageContext.getMessagePojo().getMessageId() + " (" + messageContext.getMessagePojo().getConversation().getChoreography().getName() + " - " + messageContext.getMessagePojo().getAction() + ")", messageContext.getMessagePojo()));
                        }
                    } else {
                        LOG.debug(new LogMessage("No status update pipelines configured!",
                                messageContext.getMessagePojo()));
                    }
            ...
            LOG.info(new LogMessage("Stopped InboundQueueListener (StatusUpdateSerializer) " + StatusUpdateSerializer.this.choreographyId, (messageContext == null ? null : messageContext.getMessagePojo())));
            stopRequested = false;
        } // run
