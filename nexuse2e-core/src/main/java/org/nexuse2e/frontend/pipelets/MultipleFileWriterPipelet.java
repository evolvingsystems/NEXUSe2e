...
public class MultipleFileWriterPipelet extends AbstractPipelet {
            ...
            DefaultHandler handler = new DefaultHandler() {
                ...
                public void startElement(String uri, String localName, String name, Attributes attributes) throws SAXException {
                    if ("Writer".equals(name) && attributes != null) {
                        ...
                        if (StringUtils.isNotEmpty(choreographyId) && StringUtils.isNotEmpty(actionId) && StringUtils.isNotEmpty(partnerId)) {
                            try {
                                MultipleFileWriterPipelet.this.config.put(new ConfigKey(choreographyId, actionId,
                                        partnerId), new WriterConfig(attributes.getValue("", "dir"),
                                        Boolean.parseBoolean(attributes.getValue("", "useContentId")),
                                        attributes.getValue("", "filePrefix"), attributes.getValue("", "fileExtension"
                                ), attributes.getValue("", "tmpDir"), attributes.getValue("", "tmpFileNameExt"),
                                        attributes.getValue("", "timestampPattern"), choreographyId, actionId,
                                        partnerId));
                            } catch (IllegalArgumentException e) {
                                LOG.error("Cannot initialize directory scanner number " + writerCounter + " defined " +
                                        "in the config file. Please check your configuration.");
                            }
                        } else {
                            LOG.warn("Missing attribute choreographyId, actionId, or partner in writer config. That " +
                                    "entry will be ignored. Check your configuration file.");
                        }
                    }
                }
    ...
    public MessageContext processMessage(MessageContext messageContext) throws IllegalArgumentException,
        ...
        if (config != null) {
            if (messageContext != null && messageContext.getMessagePojo() != null && messageContext.getChoreography() != null && messageContext.getPartner() != null) {
                ...
                WriterConfig currConfig = config.get(new ConfigKey(choreographyId, actionId, partnerId));
                if (currConfig != null) {
                    // render timestamp
                    ...
                    for (MessagePayloadPojo currPayload : messageContext.getMessagePojo().getMessagePayloads()) {
                        if (currPayload != null) {
                            ...
                            // render file name
                            String newFileName = currConfig.getFilePrefix() + (currConfig.isUseContentId() ?
                                    contentId : "") + (timestamp != null ? "_" + timestamp : "") + currConfig.getFileExtension();
                            ...
                            if (file.exists()) {
                                LOG.warn(new LogMessage("Overwriting existing file " + file.getAbsolutePath() + " " +
                                        "with payload  " + currPayload.getSequenceNumber() + " (" + currPayload.getContentId() + ")", messageContext));
                            }
                            try {
                                // write file
                                ...
                                if (useTmpDir || useTmpFileExt) {
                                    ...
                                    // log warning, if file exists alread (e.g. for multiple payloads without content
                                    // id)
                                    if (finalDestFile.exists()) {
                                        LOG.warn(new LogMessage("Overwriting existing file " + finalDestFile.getAbsolutePath() + " with payload  " + currPayload.getSequenceNumber() + " (" + currPayload.getContentId() + ")", messageContext));
                                    }
                                    try {
                                        if (!file.renameTo(finalDestFile)) {
                                            throw new NexusException(new LogMessage("Could not rename file " + file.getAbsolutePath() + " to " + finalDestFile + ".", messageContext));
                                        }
                                    } catch (SecurityException e) {
                                        throw new NexusException(new LogMessage("Could not rename file " + file.getAbsolutePath() + " to " + finalDestFile + ": " + e.getMessage(), messageContext), e);
                                    }
                                }
                            } catch (Exception e) {
                                throw new NexusException(new LogMessage("Error writing payload to file: " + e.getMessage(), messageContext), e);
                            }
                        }
                    }
                } else {
                    throw new NexusException(new LogMessage("No file writer configuration found for " + choreographyId + "/" + actionId + "/" + partnerId + ".", messageContext));
                }
            } else {
                throw new NexusException(new LogMessage("Incomplete message context. Message, choreography, or " +
                        "partner missing.", messageContext));
            }
        } else {
    ...
    class WriterConfig {
        ...
        private WriterConfig(String dir, boolean useContentId, String filePrefix, String fileExtension, String tmpDir
                , String tmpFileNameExt, String timestampPattern, String choreographyId, String actionId,
                             String partnerId) {
            super();
