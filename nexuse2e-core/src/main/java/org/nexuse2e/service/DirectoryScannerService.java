...
public class DirectoryScannerService extends AbstractService implements SchedulerClient {
    ...
    public void fillParameterMap(Map<String, ParameterDescriptor> parameterMap) {
        ...
        parameterMap.put(DIRECTORY, new ParameterDescriptor(ParameterType.STRING, "Directory", "The directory to scan" +
                " for files.", ""));
        parameterMap.put(BACKUP_DIRECTORY, new ParameterDescriptor(ParameterType.STRING, "Backup Directory", "The " +
                "directory to backup files to (optional).", ""));
        parameterMap.put(INTERVAL, new ParameterDescriptor(ParameterType.STRING, "Interval", "Interval inbetween " +
                "directory scans (millseconds)", "5000"));
        parameterMap.put(CHOREOGRAPHY, new ParameterDescriptor(ParameterType.STRING, "Choreography", "The " +
                "choreography to use.", ""));
        parameterMap.put(ACTION, new ParameterDescriptor(ParameterType.STRING, "Action", "The action to trigger.", ""));
        parameterMap.put(PARTNER, new ParameterDescriptor(ParameterType.STRING, "Partner", "The partner to send the " +
                "message.", ""));
        parameterMap.put(FILTER, new ParameterDescriptor(ParameterType.STRING, "Extension", "File extension to limit " +
                "processing to.", ""));
        parameterMap.put(CONVERSATION, new ParameterDescriptor(ParameterType.STRING, "Conversation", "The XPATH " +
                "statement to create the conversation ID to use.", ""));

        parameterMap.put(MAPPING_SERVICE, new ParameterDescriptor(ParameterType.SERVICE, "Mapping Service", "The " +
                "Mapping and Conversion Service.", DataConversionService.class));
    }
    ...
    public void initialize(EngineConfiguration config) throws InstantiationException {
        ...
        if (!StringUtils.isEmpty(serviceName)) {
            ...
            if (service != null && service instanceof DataConversionService) {
                ...
                throw new InstantiationException("the selected serviceName: " + serviceName + " references no valid " +
    ...
    public void scheduleNotify() {
        ...
        try {
            ...
            for (int i = 0; i < files.length; i++) {
                if (files[i].isFile() && files[i].canWrite() && (files[i].length() != 0 /* work around for files not
                completely ready/empty */)) {
                    try {
        ...
        if ((newFile != null) && (newFile.length() != 0)) {
            try {
                ...
                /*
                 ...
                 newPayload.setContentType( "text/xml" ); // Default to text/xml in case the time could not be
                 determined
                 }
                 */
                ...
                if (partnerId.startsWith("${")) {
                    if (mappingService == null) {
                        LOG.error("no valid Mapping service configured. Mapping and conversion features are not " +
                                "available!");
                    } else {
                ...
                backendPipelineDispatcher.processMessage(tempPartnerId, tempChoreographyId, tempActionId,
                        conversationId, label, null, fileBuffer);
