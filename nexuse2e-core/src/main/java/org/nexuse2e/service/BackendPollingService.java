...
public class BackendPollingService extends AbstractService implements SchedulerClient {
    ...
    public void fillParameterMap(Map<String, ParameterDescriptor> parameterMap) {
        ...
        parameterMap.put(TEMPLATE_PARAM_NAME, new ParameterDescriptor(ParameterType.STRING, "Template file location",
                "The request template file pattern. May contain variables (e.g. " + ServerProperty.DOCUMENT_TYPE.getValue() + ")", ""));
        parameterMap.put(TRP_TRANSPORT_PARAM_NAME, new ParameterDescriptor(ParameterType.STRING, "TRP transport",
                "The TRP transport protocol identifier", ""));
        parameterMap.put(TRP_PROTOCOL_PARAM_NAME, new ParameterDescriptor(ParameterType.STRING, "TRP protocol", "The " +
                "TRP application-level protocol identifier", ""));
        parameterMap.put(TRP_VERSION_PARAM_NAME, new ParameterDescriptor(ParameterType.STRING, "TRP version", "The " +
                "TRP version number", ""));
    }
    ...
    public void poll() throws NexusException {
        ...
        ConfigurationAccessService cas = Engine.getInstance().getActiveConfigurationAccessService();
        TRPPojo trp = cas.getTrpByProtocolVersionAndTransport((String) getParameter(TRP_PROTOCOL_PARAM_NAME),
                (String) getParameter(TRP_VERSION_PARAM_NAME), (String) getParameter(TRP_TRANSPORT_PARAM_NAME));
                    ...
                    for (ActionPojo action : choreography.getActions()) {
                        if (action.isPollingRequired()) {
                            if (LOG.isTraceEnabled()) {
                                LOG.trace("Polling for partner " + partner.getPartnerId() + " on choreography " + choreography.getName() + ", documentType is " + action.getDocumentType());
                            }
                            ...
                            if (f == null || !f.equals(s)) {
                                ...
                                try {
                                    if (!file.exists()) {
                                        throw new FileNotFoundException("File " + file.getAbsolutePath() + " does not" +
                                                " exist");
                                    }
                            ...
                            cas.getBackendPipelineDispatcher().processMessage(partner.getPartnerId(),
                                    choreography.getName(), action.getName(), conversationId, messageId, null, null,
                                    data);
