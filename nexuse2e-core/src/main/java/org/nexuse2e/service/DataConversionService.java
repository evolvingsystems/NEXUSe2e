...
public class DataConversionService extends AbstractService {
    ...
    private String processValidatePattern(String[] params, MappingDefinition definition,
        ...
        if (params == null || params.length < 3) {
            LOG.error("pattern validation requires at least 3 parameter! e.g: validatePattern[$value,'decimal','.'," +
                    "'true'] / validatePattern[$value,'numeric','true']");
            return null;
        }
     ...
     * @return
     */
    private String processSubstring(String[] params, MappingDefinition definition,
                                    Map<String, String> additionalValues) {
     ...
     * @return
     */
    private String processDBMapping(String category, boolean left, String[] params,
                                    Map<String, String> additionalValues) {
        ...
        MappingPojo pojo =
                Engine.getInstance().getActiveConfigurationAccessService().getMappingByCategoryDirectionAndKey(category, left, additionalValues.get("$value"));
        if (pojo != null) {
     ...
     * @param additionalValues
     */
    private String processDateFormat(String[] params, MappingDefinition definition,
                                     Map<String, String> additionalValues) {

        if (params == null || params.length < 2) {
            LOG.error("dateformat requires at least 2 parameters: dateformat['sourceformat','targetformat'], third " +
                    "parameter timezone is optional. '-0300' or 'local'");
            return null;
        }
        ...
        try {
            ...
            if (isVariable(params[0])) {
                ...
                if (dateValue.substring(dateValue.length() - 3).startsWith(":")) {
                    ...
                    String newDate =
                            dateValue.substring(0, dateValue.length() - 3) + dateValue.substring(dateValue.length() - 2);
                    dateValue = newDate;
                }
     ...
     * 			     The first element is the input pattern for parsing the original number (as understood by
     * 			     {@link java.text.DecimalFormat}).
     *               The second element is the language code for which the input pattern is valid (must be a language
     *               code as specified by ISO-639).
     * 				 The third element is the output pattern for the number after conversion (as understood by
     * 				 {@link java.text.DecimalFormat}).
     *               The fourth element is the language code for which the output pattern is valid (must be a
     *               language code as specified by ISO-639).
     * @param additionalValues A mapping that should contain at least the value (mapped as <code>"$value"</code>)
     *                         that should be the operand. It must be a number in the format of the input pattern
     *                         (<code>params[0]</code>)
     *                         which is valid for the specified language (<code>params[1]</code>).
     * @return The converted number or the value of <code>$value</code>, or <code>$value</code>, if it is
    ...
    private String processNumberFormat(String[] params, Map<String, String> additionalValues) throws DataConversionException {
        ...
        if (value != null && value.length() > 0) {
            if (params.length >= 4) {
                ...
                try {
                    result =
                            new DecimalFormat(pattern_out, new DecimalFormatSymbols(new Locale(lang_code_out))).format(new DecimalFormat(pattern_in, new DecimalFormatSymbols(new Locale(lang_code_in))).parse(value));
                } catch (Exception e) {
                    throw new DataConversionException("Cannot convert " + value + " from " + pattern_in + " (" + lang_code_in + ") to " + pattern_out + " (" + lang_code_out + ")", e);
                }
            } else {
                LOG.error("numberFormat requires at least 4 parameters: numberFormat['pattern_in','lang_code_in'," +
                        "'pattern_out','lang_code_out'] (patterns as understood by java.text.DecimalFormat, language " +
                        "codes as specified by ISO-639)");
            }
        } else {
            result = value;
        }
     ...
     *               The fourth element is the language code for which the output pattern is valid (must be a
     *               language code as specified by ISO-639).
     *               The fifth element is the operand for the operation. It is assumed that it is in the format of
     *               the input pattern/language.
     * @param additionalValues A mapping that should contain at least the value (mapped as <code>"$value"</code>)
     *                         that should be the other operand. It must be a number in the format of the input
     *                         pattern (<code>params[0]</code>)
     *                         which is valid for the specified language (<code>params[1]</code>).
     * @return The sum of the value of <code>$value</code> and the scalar number, or <code>$value</code>, if it is
     * <code>null</code> or an empty string.
     * @throws DataConversionException if an error occurs during conversion.
     */
    private String processAdd(String[] params, Map<String, String> additionalValues) throws DataConversionException {
        ...
        if (value != null && value.length() > 0) {
            if (params.length >= 5) {
                ...
                try {
                    Number valueNumber = new DecimalFormat(pattern_in,
                            new DecimalFormatSymbols(new Locale(lang_code_in))).parse(value);
                    Number operandValue = new DecimalFormat(pattern_in,
                            new DecimalFormatSymbols(new Locale(lang_code_in))).parse(operand);
                    double resultValue = valueNumber.doubleValue() + operandValue.doubleValue();
                    result =
                            new DecimalFormat(pattern_out, new DecimalFormatSymbols(new Locale(lang_code_out))).format(resultValue);
                } catch (Exception e) {
                    throw new DataConversionException("Cannot add " + operand + " to " + value, e);
                }
            } else {
                LOG.error("add requires at least 5 parameters: add['pattern_in','lang_code_in','pattern_out'," +
                        "'lang_code_out','operand'] (patterns as understood by java.text.DecimalFormat, language " +
                        "codes as specified by ISO-639; it is assumed that the operand is in format of " +
                        "pattern_in/lang_code_in)");
            }
        } else {
            result = value;
        }
     ...
     *                         pattern (<code>params[0]</code>)
     *                         which is valid for the specified language (<code>params[1]</code>).
     * @return The difference of the value of <code>$value</code> and the scalar number, or <code>$value</code>, if
     * it is <code>null</code> or an empty string.
     * @throws DataConversionException if an error occurs during conversion.
     */
    private String processSubtract(String[] params, Map<String, String> additionalValues) throws DataConversionException {
        ...
        if (value != null && value.length() > 0) {
            if (params.length >= 5) {
                ...
                LOG.error("subtract requires at least 5 parameters: subtract['pattern_in','lang_code_in'," +
                        "'pattern_out','lang_code_out','operand'] (patterns as understood by java.text.DecimalFormat," +
                        " language codes as specified by ISO-639; it is assumed that the operand is in format of " +
                        "pattern_in/lang_code_in)");
            }
        } else {
            result = value;
     ...
     *                         pattern (<code>params[0]</code>)
     *                         which is valid for the specified language (<code>params[1]</code>).
     * @return The product of the value of <code>$value</code> and the scalar number, or <code>$value</code>, if it
    ...
    private String processMultiply(String[] params, Map<String, String> additionalValues) throws DataConversionException {
        ...
        if (value != null && value.length() > 0) {
            if (params.length >= 5) {
                ...
                LOG.error("multiply requires at least 5 parameters: multiply['pattern_in','lang_code_in'," +
                        "'pattern_out','lang_code_out','operand'] (patterns as understood by java.text.DecimalFormat," +
                        " language codes as specified by ISO-639; it is assumed that the operand is in format of " +
                        "pattern_in/lang_code_in)");
            }
     ...
     *                         pattern (<code>params[0]</code>)
     *                         which is valid for the specified language (<code>params[1]</code>).
     * @return The quotient of the value of <code>$value</code> and the scalar number, or <code>$value</code>, if it
     * is <code>null</code> or an empty string.
     * @throws DataConversionException if an error occurs during conversion.
     */
    private String processDivide(String[] params, Map<String, String> additionalValues) throws DataConversionException {
        ...
        if (value != null && value.length() > 0) {
            if (params.length >= 5) {
                ...
                LOG.error("divide requires at least 5 parameters: divide['pattern_in','lang_code_in','pattern_out'," +
                        "'lang_code_out','operand'] (patterns as understood by java.text.DecimalFormat, language " +
                        "codes as specified by ISO-639; it is assumed that the operand is in format of " +
                        "pattern_in/lang_code_in)");
            }
