    ...
    public MessageContext sendMessage(MessageContext messageContext) throws NexusException {
        ...
        try {
            ...
            if (trpPojo.getProtocol().equalsIgnoreCase(org.nexuse2e.Constants.PROTOCOL_ID_EBXML)) {
                ...
                // TODO (encoding) http plain should use ByteArrayRequestEntity / InputStreamRequestEnity? Content
                //  Type ?
            ...
            LogMessage lm =
                    new LogMessage("Message submission failed, connection timeout for URL: " + messageContext.getParticipant().getConnection().getUri() + " - " + e, messageContext.getMessagePojo());
            LOG.warn(lm, e);
