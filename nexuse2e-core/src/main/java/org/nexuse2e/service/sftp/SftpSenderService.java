 ...
 *
 * @author jonas.reese
 * @version $LastChangedRevision: 162 $ - $LastChangedDate: 2007-07-30 13:52:13 +0000 (Mo, 30 Jul 2007) $ by
 * $LastChangedBy: gesch $
 */
public class SftpSenderService extends AbstractService implements SenderAware {
    ...
    public void fillParameterMap(Map<String, ParameterDescriptor> parameterMap) {
        ...
        parameterMap.put(TEMP_FILE_PARAM_NAME, new ParameterDescriptor(ParameterType.BOOLEAN, "Use temporary file " +
                "name", "Use temporary file name during upload", Boolean.TRUE));

        parameterMap.put(APPEND_TIMESTAMP, new ParameterDescriptor(ParameterType.BOOLEAN, "Append Timestamp", "Flag " +
                "whether to append a timestamp to the filename", Boolean.TRUE));

        parameterMap.put(TIMESTAMP_PATTERN, new ParameterDescriptor(ParameterType.STRING, "Timestamp Pattern", "The " +
                "pattern of the timestamp that should be appended. Use pattern syntax of <a href=\"http://java.sun" +
                ".com/j2se/1.5.0/docs/api/java/text/SimpleDateFormat.html\">SimpleDateFormat</a>.",
                DEFAULT_TIMESTAMP_PATTERN));

        parameterMap.put(PRIVATE_KEY, new ParameterDescriptor(ParameterType.STRING, "Private Key File", "File that " +
                "contains the private key (DSA or RSA) for SFTP authentication in PEM format. ", ""));
    }
    ...
    public MessageContext sendMessage(MessageContext messageContext) throws NexusException {
        ...
        try {
            ...
            try {
                ...
                throw new NexusException(new LogMessage("SFTP connection/authentication failed",
                        messageContext.getMessagePojo()), jSchEx);
            }
            ...
            if (StringUtils.isNotEmpty(directory)) {
                ...
                try {
                    ...
                    throw new NexusException(new LogMessage("SFTP server did not change directory",
                            messageContext.getMessagePojo()), sftpEx);
                }
            }
