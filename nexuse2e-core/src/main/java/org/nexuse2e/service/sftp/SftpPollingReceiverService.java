 ...
 * @version $LastChangedRevision: 262 $ - $LastChangedDate: 2007-09-18 11:58:43 +0000 (Di, 18 Sep 2007) $ by
 * $LastChangedBy: mbreilmann $
 */
public class SftpPollingReceiverService extends AbstractService implements ReceiverAware {
    ...
    public void fillParameterMap(Map<String, ParameterDescriptor> parameterMap) {
        ...
        parameterMap.put(PARTNER_PARAM_NAME, new ParameterDescriptor(ParameterType.STRING, "Partner", "The partner " +
                "ID", ""));
        ...
        parameterMap.put(FILE_PATTERN_PARAM_NAME, new ParameterDescriptor(ParameterType.STRING, "File pattern", "DOS" +
                "-like file pattern with wildcards * and ? (examples: *.xml, ab?.*)", "*"));
        ...
        parameterMap.put(DSA_PRIVATE_KEY_PATH_PARAM_NAME, new ParameterDescriptor(ParameterType.STRING, "Private key " +
                "path", "The path to the SFTP private key", "anonymous"));

        parameterMap.put(DSA_PRIVATE_KEY_PASSWORD_PARAM_NAME, new ParameterDescriptor(ParameterType.PASSWORD,
                "Private key password", "The password of the SFTP private key", ""));

        parameterMap.put(SFTP_PORT_PARAM_NAME, new ParameterDescriptor(ParameterType.STRING, "Server port", "The port" +
                " of the SFTP-Server", "22"));
            ...
            try {
                ...
                if (StringUtils.isNotEmpty(sftpPortStr)) {
                    try {
                        ...
                        throw new NexusException("Could not parse SFTP port parameter, parameter needs to be a " +
                                "number: " + sftpPortStr, e);
                    }
                }
                        ...
                        if (!fileChangeActive || (targetFiles == null) || targetFiles.isEmpty()) {
                            ...
                            try {
                                ...
                                try {
                                    ...
                                    boolean processedSucessfully = processFile(localFile, errorDir,
                                            (String) getParameter(PARTNER_PARAM_NAME));
                                    ...
                                    if (processedSucessfully && fileChangeActive) {
                                        if (StringUtils.isEmpty(prefix)) {
                                            ...
                                            try {
                                                ...
                                                LOG.error("Could not rename file from " + file.getFilename() + " to " + prefix + file.getFilename());
                                                error = true;
                                            }
                                        }
                                    }
