...
public class MessageMappingPipelet extends AbstractPipelet {
    ...
    public MessageMappingPipelet() {
        ...
        parameterMap.put(ACTION_MAP_CATEGORY_PARAMETER_NAME, new ParameterDescriptor(ParameterType.STRING,
                "Action " + "map category name", "Enter category name or leave blank if actions shall not be mapped",
                ""));
        parameterMap.put(CHOREOGRAPHY_MAP_CATEGORY_PARAMETER_NAME, new ParameterDescriptor(ParameterType.STRING,
                "Choreography map category name", "Enter category name or leave blank if choreographies shall not be "
                + "mapped", ""));
        parameterMap.put(PARTNER_MAP_CATEGORY_PARAMETER_NAME, new ParameterDescriptor(ParameterType.STRING, "Partner "
                + "map category name", "Enter category name or leave blank if partners shall not be mapped", ""));
    }
    ...
    public MessageContext processMessage(MessageContext messageContext) throws IllegalArgumentException,
        ...
        if (messageContext.getMessagePojo() == null) {
            // nothing we can do
            LOG.warn(getClass().getSimpleName() + " cannot process message because message is not set on message " +
                    "context");
        ...
        if (!StringUtils.isBlank(choreographyCategory)) {
            ...
            if (messageContext.getMessagePojo().getConversation() != null) {
                ...
                if (choreography != null) {
                    MappingPojo mp =
                            Engine.getInstance().getCurrentConfiguration().getMappingByCategoryDirectionAndKey(choreographyCategory, true, choreography.getName());
                    String mappedChoreography = (mp == null ? null : mp.getRightValue());
                    if (!StringUtils.isBlank(mappedChoreography)) {
                        // do it
                        ChoreographyPojo newChoreography =
                                Engine.getInstance().getCurrentConfiguration().getChoreographyByChoreographyId(mappedChoreography);
                        if (newChoreography != null) {
                            // set new choreography on message and message context
                            if (messageContext.getActionSpecificKey() != null) {
                                messageContext.setActionSpecificKey(new ActionSpecificKey(messageContext.getActionSpecificKey().getActionId(), mappedChoreography));
                            }
                            if (messageContext.getMessagePojo().getConversation() != null) {
                            ...
                            LOG.warn(getClass().getSimpleName() + " cannot map choreography because no choreography " + "with ID " + mappedChoreography + " was found");
                        }
                    } else {
                        LOG.warn(getClass().getSimpleName() + " cannot map action because mapping for key " + choreography.getName() + " and category " + choreographyCategory + " was not found");
                    }
                } else {
                    LOG.warn(getClass().getSimpleName() + " cannot map choreography because no choreography is set " +
                            "on" + " conversation");
                }
            } else {
        ...
        if (!StringUtils.isBlank(actionCategory)) {
            ...
            if (action != null && choreography != null) {
                MappingPojo mp =
                        Engine.getInstance().getCurrentConfiguration().getMappingByCategoryDirectionAndKey(actionCategory, true, action.getName());
                String mappedAction = (mp == null ? null : mp.getRightValue());
                if (!StringUtils.isBlank(mappedAction)) {
                    // do it
                    ActionPojo newAction =
                            Engine.getInstance().getCurrentConfiguration().getActionFromChoreographyByActionId(choreography, mappedAction);
                    if (newAction != null) {
                        // set new action on message and message context
                        if (messageContext.getActionSpecificKey() != null) {
                            messageContext.setActionSpecificKey(new ActionSpecificKey(mappedAction,
                                    messageContext.getActionSpecificKey().getChoreographyId()));
                        }
                        messageContext.getMessagePojo().setAction(newAction);
                    } else {
                        LOG.warn(getClass().getSimpleName() + " cannot map action because no action with ID " + mappedAction + " was found in choreography " + messageContext.getMessagePojo().getConversation().getChoreography().getName());
                    }
                } else {
        ...
        if (!StringUtils.isBlank(partnerCategory)) {
            ...
            if (participant != null && messageContext.getConversation() != null) {
                ...
                if (partner != null) {
                    MappingPojo mp =
                            Engine.getInstance().getCurrentConfiguration().getMappingByCategoryDirectionAndKey(partnerCategory, true, partner.getPartnerId());
                    String mappedPartner = (mp == null ? null : mp.getRightValue());
                    if (!StringUtils.isBlank(mappedPartner)) {
                        PartnerPojo newPartner =
                                Engine.getInstance().getCurrentConfiguration().getPartnerByPartnerId(mappedPartner);
                        if (newPartner != null && messageContext.getMessagePojo().getConversation().getChoreography() != null) {
                            ParticipantPojo newParticipant =
                                    Engine.getInstance().getCurrentConfiguration().getParticipantFromChoreographyByPartner(messageContext.getMessagePojo().getConversation().getChoreography(), newPartner);
                            if (newParticipant != null) {
                                ...
                                LOG.warn(getClass().getSimpleName() + " cannot map partner because no participant " + "with partner ID " + mappedPartner + " is configured for choreography " + messageContext.getMessagePojo().getConversation().getChoreography().getName());
                            }
                        } else {
                            if (newPartner == null) {
                                LOG.warn(getClass().getSimpleName() + " cannot map partner because no partner with " + "partner ID " + mappedPartner + " was found");
                            } else {
                                LOG.warn(getClass().getSimpleName() + " cannot map partner because no choreography " +
                                        "is" + " set on conversation");
                            }
                        }
                    } else {
                        LOG.warn(getClass().getSimpleName() + " cannot map partner because mapping for key " + partner.getPartnerId() + " and category " + partnerCategory + " was not found");
                    }
                } else {
                    LOG.warn(getClass().getSimpleName() + " cannot map partner because no partner is set on message");
                }
            } else {
                LOG.warn(getClass().getSimpleName() + " cannot map partner because no " + (messageContext.getMessagePojo().getParticipant() == null ? "participant" : "conversation") + "is set");
            }
        }
