...
public class DBIntegrationPipelet extends AbstractPipelet {
    ...
    private static Logger LOG = LoggerFactory.getLogger(DBIntegrationPipelet.class);
    ...
    public DBIntegrationPipelet() {
        ...
        parameterMap.put(DATABASESERVICE, new ParameterDescriptor(ParameterType.SERVICE, "Database Service", "The " +
                "name of the service that shall be used for database connection pooling", ""));
        parameterMap.put(TABLENAME, new ParameterDescriptor(ParameterType.STRING, "Table name", "The name of the " +
                "table used for backend integration", ""));
    ...
    public MessageContext processMessage(MessageContext messageContext) throws IllegalArgumentException,
        ...
        String sql = "INSERT INTO " + tableName + " ( ChoreographyID, ConversationID, MessageID, ActionID, " +
                "ParticipantID, InboundFlag, " + "ContentType, PayloadField ,Source,Created_Date,LastModified_Date) " + "values( ?, ?, ?, ?, ?, ?, ?,?,?,?,?)";
        ...
        for (MessagePayloadPojo pojo : payloads) {
            ...
            try {
                ...
                LOG.error("Error processing inbound message, Conversation ID  " + messageContext.getMessagePojo().getConversation().getConversationId(), exception);
            } finally {
                try {
