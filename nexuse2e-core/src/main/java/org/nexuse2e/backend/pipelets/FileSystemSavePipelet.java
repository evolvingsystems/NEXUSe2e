...
public class FileSystemSavePipelet extends AbstractPipelet {
    ...
    public FileSystemSavePipelet() {
        ...
        parameterMap.put(DIRECTORY_PARAM_NAME, new ParameterDescriptor(ParameterType.STRING, "Save directory", "Path "
                + "to directory where to store files", ""));
        ...
        parameterMap.put(USE_CONTENT_ID, new ParameterDescriptor(ParameterType.BOOLEAN, "Use Content ID", "Flag " +
                "whether to use the content ID as the file name", Boolean.FALSE));
     ...
     * @throws IOException
     */
    private String writePayload(String destinationDirectory, MessagePayloadPojo payload,
                                MessageContext messageContext) throws FileNotFoundException, IOException {
        ...
        if (useContentId) {
            ...
            fileName =
                    destinationDirectory + File.separatorChar + baseFileName + "_" + payload.getSequenceNumber() +
                            "." + extension;
        }
