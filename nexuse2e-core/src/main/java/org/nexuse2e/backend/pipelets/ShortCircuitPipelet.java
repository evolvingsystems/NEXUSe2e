...
public class ShortCircuitPipelet extends AbstractPipelet {
    ...
    public ShortCircuitPipelet() {
        ...
        parameterMap.put(PARAM_SYNCHRONOUS, new ParameterDescriptor(ParameterType.BOOLEAN, "Synchronous",
                "Process " + "return message in same thread", Boolean.TRUE));
        parameterMap.put(PARAM_SYNCHONOUS_DELAY, new ParameterDescriptor(ParameterType.STRING, "Inbound pipeline " +
                "delay", "Time to wait until returning control to backend inbound pipeline (in ms)", "0"));
        parameterMap.put(PARAM_DELAY, new ParameterDescriptor(ParameterType.STRING, "Return message delay", "Time to "
                + "wait until sending return message (in ms)", "0"));
    }
        ...
        Runnable r = new Runnable() {
            ...
            public void run() {
                ...
                if (backendPipelineDispatcher != null) {
                    try {
                        backendPipelineDispatcher.processMessage(messageContext.getMessagePojo().getConversation().getConversationId(), action, null, "<?xml version=\"1.0\" encoding=\"UTF-8\"?><test />".getBytes());
