...
public class ParticipantTargetProvider implements TargetProvider {
    ...
    public List<StructureNode> getStructure(StructureNode pattern, ParentalStructureNode parent,
        ...
        try {
            ChoreographyPojo choreographyPojo =
                    engineConfiguration.getChoreographyByNxChoreographyId(Integer.parseInt(parent.getProperty(
                            "nxChoreographyId")));
            if (choreographyPojo != null) {
                Collection<ParticipantPojo> participants = choreographyPojo.getParticipants();
                TreeSet<ParticipantPojo> sortedParticipants =
                        new TreeSet<ParticipantPojo>(new GenericComparator<ParticipantPojo>("description;partner" +
                                ".partnerId", true));
                sortedParticipants.addAll(participants);
                for (ParticipantPojo participantPojo : sortedParticipants) {
                    StructureNode sn =
                            new PageNode(pattern.getTarget() + "?nxPartnerId=" + participantPojo.getPartner().getNxPartnerId() + "&nxChoreographyId=" + choreographyPojo.getNxChoreographyId(), participantPojo.getPartner().getPartnerId() + " (" + participantPojo.getPartner().getName() + ")", pattern.getIcon());
                    list.add(sn);
                }

            }
        } catch (NullPointerException e) {
            LOG.error("NullPointerException while rendering participant tree - most likely cause is a missing " +
                    "description value in a participant entry: " + e);
            e.printStackTrace();
        } catch (Exception e) {
