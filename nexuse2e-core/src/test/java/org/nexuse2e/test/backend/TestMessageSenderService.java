...
public class TestMessageSenderService extends AbstractService implements SchedulerClient {
    ...
    public void fillParameterMap(Map<String, ParameterDescriptor> parameterMap) {
        ...
        parameterMap.put(SCHEDULINGSERVICE, new ParameterDescriptor(ParameterType.SERVICE, "Scheduling Service", "The" +
                " name of the service that shall be used for time schedule", ""));
        parameterMap.put(CHOREOGRAPHY, new ParameterDescriptor(ParameterType.STRING, "Choreography", "The name of the" +
                " Choreography used for testing", ""));
        parameterMap.put(ACTION, new ParameterDescriptor(ParameterType.STRING, "Action", "The Action used for " +
                "testing", ""));
        parameterMap.put(PARTNER, new ParameterDescriptor(ParameterType.STRING, "Partner", "The Partner used for " +
                "testing", ""));
        parameterMap.put(INTERVAL, new ParameterDescriptor(ParameterType.STRING, "Interval", "Interval inbetween test" +
                " messages (Millseconds)", "5000"));
    }
    ...
    public void scheduleNotify() {
        ...
        try {
            messageContext =
                    Engine.getInstance().getCurrentConfiguration().getBackendPipelineDispatcher().processMessage(partner, choreography, action, null, null, null, DEFAULT_PAYLOAD.getBytes());
